...
:root {
    ...
    --bg-content-purple: rgba(45, 25, 55, 0.97); /* Primary content background */
...
/* Basic Body Styling */
html {
    height: 100%; /* Ensure html takes full height for 100vh on body to work correctly */
...
#svg-background-container {
    ...
    z-index: -10; 
    overflow: hidden; 
    background-color: var(--bg-dark-purple); /* Fallback solid color */
}

#svg-background { /* Style your SVG element here */
    width: 100%;
    height: 100%;
...
nav .nav-links {
    ...
    align-items: center;
    flex-wrap: nowrap; 
    overflow-x: auto; /* Allows nav links to scroll if they don't fit */
    -webkit-overflow-scrolling: touch; /* Smooth scrolling on iOS */
} 
/* Hide scrollbar for nav-links if it appears */
...
nav .nav-links button, /* For auth buttons if they are actual <button> elements */
nav .nav-links .auth-button, /* If auth buttons are styled <a> or <button> with this class */
...
main.content-section-wrapper { 
    flex-grow: 1; /* Takes up available space between nav and footer */
    display: flex; /* Important for the scroller child */
    overflow: hidden; /* This wrapper should not have scrollbars */
    position: relative; 
    background-color: var(--bg-dark-purple); /* Or a slightly different shade if desired */
}

/* Page Content Scroller (The element that actually scrolls horizontally) */
.page-content-scroller { 
    display: flex; /* Lays out tiles horizontally */
    flex-direction: row;
    width: 100%; /* Takes full width of its parent */
    height: 100%; /* Takes full height of its parent */
    overflow-x: auto; /* Enables horizontal scrolling */
    overflow-y: hidden; /* Prevents vertical scrolling on the scroller itself */
    scroll-snap-type: x mandatory; /* Snaps to tiles */
    scroll-behavior: smooth;
    -webkit-overflow-scrolling: touch; /* Smooth scrolling on iOS */
}
/* Hide scrollbar for the scroller */
...
.content-section-tile {
    width: 100vw; /* Each tile takes full viewport width */
    height: 100%; /* Full height of the scroller */
    flex-shrink: 0; /* Prevents tiles from shrinking */
    padding: 1.5rem 2rem; /* Inner padding for content within the tile */
    overflow-y: auto; /* Allows vertical scroll *within this tile* if its content is too long */
    scroll-snap-align: start; /* Aligns tile to the start when snapping */
    background-color: var(--bg-content-purple); /* Background for the tile content area */
    box-sizing: border-box;
    color: var(--text-primary-light);
    /* Optional: border between tiles if needed */
    /* border-right: 1px solid var(--border-color-lavender); */
}
/* Custom scrollbar for individual tiles */
...
/* Specific styling for Hero Section (if it's a tile in index.html) */
.hero-section { /* This class can be used inside a .content-section-tile */
    display: flex;
    flex-direction: column;
    align-items: center;
    ...
    text-align: center;
    padding: 2rem 1rem; 
    height: 100%; /* Make hero fill its tile */
    box-sizing: border-box;
}
.hero-section h1 {
...
/* Benefits Section Styling (if it's a tile in index.html) */
...
.benefits-background-container { /* This container might need height adjustments if inside a fixed-height tile */
    position: relative;
    ...
    justify-content: center;
    /* min-height: 300px; /* May not be needed if tile has fixed height */
    /* overflow-y: auto; /* Tile itself will handle overflow */
} 
.benefits-background-svg { /* For the decorative SVG within benefits */
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    z-index: 0;
    opacity: 0.1; /* Adjusted for less distraction */
}
.benefits-grid {
    display: grid;
...
.landing-cta-section {
    ...
    flex-direction: column;
    justify-content: center;
    height: 100%; /* To center content if this is a full-tile CTA */
    box-sizing: border-box;
}
...
/* Notifications Area */
.notifications-area { position: fixed; bottom: var(--footer-height) + 20px; /* Ensure it's above the footer */ right: 20px; width: 300px; z-index: 3000; }
...
#globalSearchTriggerBtn { position: fixed; bottom: var(--footer-height) + 20px; /* Ensure it's above the footer */ left: 20px; z-index: 1000; background-color: var(--accent-purple-medium); color: var(--text-primary-light); border: none; border-radius: 50%; width: 50px; height: 50px; font-size: 1.2rem; box-shadow: 0 4px 10px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background-color 0.3s ease, transform 0.3s ease; }
#globalSearchTriggerBtn:hover { background-color: #8e44ad; transform: scale(1.1); }

/* Footer Styling */
footer {
    text-align: center;
    padding: 0 1rem; /* Adjusted padding for height */
    color: var(--text-secondary-lavender);
    font-size: 0.8rem;
...
@media (max-width: 1024px) { 
    ...
    .hero-section h1 { font-size: clamp(1.8rem, 4vw, 2.5rem); } /* Ensure this applies within a tile */
...
@media (max-width: 768px) {
    ...
    .content-section-tile { padding: 1rem 1.5rem; } /* Adjust tile padding */
